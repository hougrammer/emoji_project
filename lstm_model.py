...
def MakeFancyRNNCell(H, keep_prob, num_layers=1):
    ...
    Returns:
      (tf.nn.rnn_cell.MultiRNNCell) multi-layer LSTM cell with dropout
    """
    cells = []
...
class RNNLM(object):
    ...
    def BuildCoreGraph(self):
        ...
        with tf.name_scope('Embedding_Layer'):
            self.W_in_ = tf.Variable(tf.random_uniform([V, H], -1.0, 1.0), name='W_in_')
            x_ = tf.nn.embedding_lookup(self.W_in_, self.input_w_, name='x')

        
        # Construct RNN/LSTM cell and recurrent layer.
        with tf.name_scope('Recurrent_Layer'):
            self.cell_ = MakeFancyRNNCell(H, self.dropout_keep_prob_, num_layers=self.num_layers)
            self.initial_h_ = self.cell_.zero_state(tf.shape(x_)[0], tf.float32)
            self.o_, self.final_h_ = tf.nn.dynamic_rnn(self.cell_, x_, initial_state=self.initial_h_)
        ...
        with tf.name_scope('Output_Layer'):
            self.W_out_ = tf.Variable(tf.random_normal([H, V]), name='W_out')
            self.b_out_ = tf.Variable(tf.zeros([V,]), name='b_out')
        ...
        with tf.name_scope('Cost_Function'):
            per_example_loss_ = tf.nn.sparse_softmax_cross_entropy_with_logits(
                labels=self.target_y_, logits=self.logits_, name='per_example_loss')
    ...
    def BuildTrainGraph(self):
        ...
        with tf.name_scope('Cost_Function'):
            per_example_train_loss_ = tf.nn.sampled_softmax_loss(weights=tf.transpose(self.W_out_), 
                                                                 ...
                                                                 inputs=tf.reshape(self.o_, [-1, tf.shape(self.o_)[2]]),
                                                                 num_sampled=self.softmax_ns, 
                                                                 num_classes=self.V,
                                                                 name='per_example_train_loss')
            self.train_loss_ = tf.reduce_mean(per_example_train_loss_, name='train_loss')


        # Define optimizer and training op
        with tf.name_scope('Training'):
            self.learning_rate_ = tf.placeholder(tf.float32, name='learning_rate')
            opt_ = tf.train.AdamOptimizer(self.learning_rate_)
            vars_ = tf.trainable_variables()
            grads_, _ = tf.clip_by_global_norm(tf.gradients(self.train_loss_, vars_), self.max_grad_norm_)
            self.train_step_ = opt_.apply_gradients(zip(grads_, vars_))
    ...
    def BuildSamplerGraph(self):
        ...
        self.pred_samples_ = tf.reshape(
            tf.multinomial(tf.reshape(self.logits_, [-1, tf.shape(self.logits_)[2]]), 1),
            [-1, self.max_time_, 1], name='pred_samples')
